...
img_size=(160, 160)
out_size=(600,400)
onnx_model_path = "onnx_models/road_seg.onnx"
num_classes = 3
...
while True:
    ...
    if ret:    
        ...
        img,mask=get_mask(frame,img_size,out_size)  # Note that the model only sees inputs at 150x150.
        #print(mask.dtype)
        (B, G, R) = cv2.split(img)
        #print(R.dtype)
        ...
        cv2.imshow('Final output onn',cv2.merge([B, G, np.bitwise_xor(R,mask.astype(np.uint8))]))
    cv2.waitKey(1)
